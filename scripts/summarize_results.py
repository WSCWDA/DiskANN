...
with open(a.output,"w",newline="") as f:
  ...
  for key,rows in sorted(groups.items()):
    ...
    duration=sum(lat)/1e6; recall=next((v for pth,v in recalls.items() if os.path.basename(pth).startswith(f"{dataset}_{backend}_L{L}_W{W}_T{T}_C{C}_{state}_r{run}")),math.nan)
