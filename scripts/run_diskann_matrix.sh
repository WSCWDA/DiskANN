...
for backend in "${backends[@]}"; do for W in "${beams[@]}"; do for T in "${threads[@]}"; do for C in "${caches[@]}"; do
  for run in $(seq 0 "${repeats}"); do
    ...
    "${build_root}/apps/search_disk_index" --data_type float --dist_fn l2 --index_path_prefix "${INDEX}" \
    ...
    command -v pidstat >/dev/null && pidstat -u -r -d -w -p "${pid}" 1 > "${stem}.pidstat" & monitor=$!
    wait "${pid}"; kill "${monitor:-0}" 2>/dev/null || true
    echo "${phase}: ${stem}"
  done
done; done; done; done
